/*
 *  exported-percpu-mod - test exported relay file ops with per-cpu buffers
 *
 ...
 *
 * This example creates per-cpu relay files in the debugfs
 * filesystem. i.e. this code is pretty much the same as the other
 * relayfs examples, with the exception that the relay files live
 * under /debug or wherever debugfs is mounted.  Relayfs itself should
 * be insmod'ed or configured in, but doesn't need to be mounted.
 *
 * Usage:
 *
 * modprobe relayfs
...
/* name of directory containing relayfs files */
char *app_dirname = "/debug/exported-percpu";
/* base name of per-cpu relayfs files (e.g. /debug/cpu0, cpu1, ...) */
...
{
	fprintf(stderr, "exported-relayfile [-b subbuf_size -n n_subbufs]\n");
	exit(1);
}
			  ...
			  const char *out_basename)
{
	...
	if (relay_file[cpu] < 0) {
		printf("Couldn't open debugfs file %s: errcode = %s\n",
		       tmp, strerror(errno));
		return -1;
	}
