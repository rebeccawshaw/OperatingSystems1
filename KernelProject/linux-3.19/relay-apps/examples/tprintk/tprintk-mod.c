/*
 *  tprintk-mod - 'tee' printk output to a relayfs channel
 *
...
/* This app's relay channel/control files appear in /debug/tprintk */
#define APP_DIR		"tprintk"
...
static struct jprobe tprintk_jp;

/*
 * printk() jprobe handler.
 */
static int tprintk(const char *fmt, ...)
{
	...
	va_start(args, fmt);
	len = vscnprintf(printk_buf, sizeof(printk_buf), fmt, args);
	/*
	 * Write the output to the relay channel.  We remove log level
	 * tags because they're not used and we write a timestamp (as
	 * [secs.usecs] so things can be sorted out between per-cpu buffers.
	 */
	for (p = start = printk_buf; *p; p++) {
...
/*
 * init - initialize relay app and insert printk probe
 *
 * relay files will be named /debug/tprintk/cpuXXX
 */
static int init(void)
{
	...
	tprintk_jp.entry = (kprobe_opcode_t *)tprintk;
	tprintk_jp.kp.symbol_name = "printk";
	register_jprobe(&tprintk_jp);
	printk("tprintk probe inserted.\n");

	return 0;
}

/*
 * cleanup - destroy channel and remove printk probe
 */
static void cleanup(void)
{
	unregister_jprobe(&tprintk_jp);
	printk("tprintk probe removed.\n");
	destroy_channel();
	remove_controls();
	if (dir)
		debugfs_remove(dir);
