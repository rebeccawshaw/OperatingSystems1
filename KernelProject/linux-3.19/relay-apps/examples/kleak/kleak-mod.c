/*
 *  kleak-mod - log kmalloc/kfree trace data
 *
...
#include <linux/klog.h> /* only needed if using klog()/klog_printk() */

/* This app's relay channel/control files will appear in /debug/kleak */
...
/*
 * log_data - klog handler implementation
 *
 * This handler is called on every klog() and/or klog_printk() call
 * from the kernel or modules, after it's been registered (by
 * register_klog_handler()).  This allows logging calls to be placed
 * anywhere in the kernel or modules even if relay isn't configured
 * in or isn't yet loaded into the kernel, and regardless of whether
 * or not there's a relay channel currently available for logging.
 * 
 * For this application, the channel is created on demand from
 * userspace, and we don't want to actually log anything until the channel
 * exists i.e. we check first before doing the write to the relay channel.
 *
 * To use klog, the klog kernel patch (klog.patch) must be applied.
 */
...
/*
 * The klog operations registed via register_klog_handler() in module init
 */
...
/**
 *	create_channel - creates channel /debug/kleak/cpuXXX
 *
...
/**
 *	destroy_channel - destroys channel /debug/kleak/cpuXXX
 *
