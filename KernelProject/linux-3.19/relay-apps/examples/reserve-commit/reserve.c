/*
 *  reserve - example of using reserve/commit with a relay channel
 *
 ...
 * insmod ./reserve-mod.ko
 * ./reserve [-b subbuf-size -n n_subbufs]
 *
...
char *app_dirname = "/debug/reserve";
...
static struct buf_status
{
	...
	size_t undelivered_subbufs; /* # undelivered sub-buffers we were
				       forced to drop */
} status[NR_CPUS];

static void usage(void)
{
	fprintf(stderr, "reserve [-b subbuf_size -n n_subbufs]\n");
	exit(1);
}
...
static int process_subbufs(unsigned int cpu)
{
	...
	for (i = start_subbuf; i < end_subbuf; i++) {
		...
		commit = *((unsigned *)subbuf_ptr);
		padding = *((unsigned *)(subbuf_ptr + sizeof(unsigned)));
		subbuf_ptr += sizeof(commit) + sizeof(padding);
		len = subbuf_size - start_reserve - padding;
		if (commit != len) {
			undelivered_idx = i;
	...
	if (subbufs_ready == n_subbufs && undelivered_idx == start_subbuf) {
...
static void summarize(void)
{
	...
	if (dropped)
		printf("\n    %zu dropped events.\n", dropped);
	printf("    %zu undelivered sub-buffers\n",
	       status[i].undelivered_subbufs);
}
